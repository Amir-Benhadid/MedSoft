...
- Name: refraction.spec.ts >> Refraction – copy buttons >> OD → OG copy button is visible
- Location: tests\e2e\refraction.spec.ts:155:5
...
Locator: getByRole('button', { name: /OD.*OG/i }).or(locator('button').filter({ hasText: 'OD' }).filter({ hasText: 'OG' }))
Expected: visible
Error: strict mode violation: getByRole('button', { name: /OD.*OG/i }).or(locator('button').filter({ hasText: 'OD' }).filter({ hasText: 'OG' })) resolved to 2 elements:
    1) <button type="button" title="Copier OG vers OD" class="inline-flex items-center justify-center whitespace-nowrap font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:text-accent-foreground rounded-md h-5 xl:h-6 2xl:h-8 px-1.5 xl:px-2 text-[10px] xl:text-xs 2xl:text-sm hover:bg-green-100">…</button> aka getByRole('button', { name: 'OG → OD' })
    2) <button type="button" title="Copier OD vers OG" class="inline-flex items-center justify-center whitespace-nowrap font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:text-accent-foreground rounded-md h-5 xl:h-6 2xl:h-8 px-1.5 xl:px-2 text-[10px] xl:text-xs 2xl:text-sm hover:bg-blue-100">…</button> aka getByRole('button', { name: 'OD → OG' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByRole('button', { name: /OD.*OG/i }).or(locator('button').filter({ hasText: 'OD' }).filter({ hasText: 'OG' }))
...
> 158 |         )).toBeVisible({ timeout: 5000 });
  ...
  216 |         const lensSection = page.getByText(/Lentilles|Type de lentille|Sphérique/i).first();
  217 |         // Page renders without crash
  218 |         expect(true).toBe(true);
  219 |     });
  220 | 
  221 |     test('selecting Sphérique auto-fills R0 and DL with defaults', async ({ page }) => {
  222 |         // Find contact lens type CompactSelect
  223 |         const spheriqueOption = page.locator('[data-value="Sphérique"]').first();
  224 |         if (await spheriqueOption.isVisible({ timeout: 2000 })) {
  225 |             await spheriqueOption.click();
  226 |             // R0 should auto-fill to 8.40
  227 |         }
  228 |     });
  229 | });
  230 | 
  231 | // ═══════════════════════════════════════════════════════════════════════════════
  232 | // Dilation workflow
  233 | // ═══════════════════════════════════════════════════════════════════════════════
  234 | test.describe('Refraction – dilation', () => {
  235 |     test.beforeEach(async ({ page }) => {
  236 |         await goToDashboard(page);
  237 |         await goToRefractionTab(page);
  238 |     });
  239 | 
  240 |     test('Dilater button appears when patient has active appointment/waitlist', async ({ page }) => {
  241 |         // This button only shows if patient has active scheduling
  242 |         const dilaterBtn = page.getByRole('button', { name: /Dilater/i });
  243 |         // Only assert if it's visible
  244 |         const isVisible = await dilaterBtn.isVisible({ timeout: 2000 }).catch(() => false);
  245 |         if (isVisible) {
  246 |             expect(dilaterBtn).toBeTruthy();
  247 |         }
  248 |     });
  249 | 
  250 |     test('clicking Dilater opens the dilation dialog', async ({ page }) => {
  251 |         const dilaterBtn = page.getByRole('button', { name: /Dilater/i });
  252 |         if (await dilaterBtn.isVisible({ timeout: 2000 })) {
  253 |             await dilaterBtn.click();
  254 |             await expect(page.getByText('Demande de dilatation')).toBeVisible({ timeout: 3000 });
  255 |         }
  256 |     });
  257 | 
  258 |     test('dilation dialog shows OD, OG, ODS buttons', async ({ page }) => {
```