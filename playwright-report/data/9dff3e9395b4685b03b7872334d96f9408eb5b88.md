...
- Name: refraction.spec.ts >> Refraction – Visual Acuity >> OD eye panel is visible with AV section
- Location: tests\e2e\refraction.spec.ts:35:5
...
Locator: getByText('OD')
Expected: visible
Error: strict mode violation: getByText('OD') resolved to 9 elements:
    1) <span class="font-bold uppercase tracking-widest text-emerald-700">OD</span> aka locator('span').filter({ hasText: 'OD' })
    2) <button type="button" title="Copier OG vers OD" class="inline-flex items-center justify-center whitespace-nowrap font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:text-accent-foreground rounded-md h-5 xl:h-6 2xl:h-8 px-1.5 xl:px-2 text-[10px] xl:text-xs 2xl:text-sm hover:bg-green-100">…</button> aka getByRole('button', { name: 'OG → OD' })
    ...
    4) <div class="h-full flex flex-col min-h-0">…</div> aka getByText('INSPMOTSEG ANTFODIAGCDT')
    5) <h4 class="text-xs font-extrabold text-white uppercase tracking-tight">OD</h4> aka getByText('OD').nth(4)
    6) <h4 class="text-xs font-extrabold text-white uppercase tracking-tight">OD</h4> aka getByText('OD').nth(5)
    7) <label class="peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-[10px] font-semibold text-emerald-700 uppercase tracking-tight w-8 shrink-0">OD</label> aka getByText('OD', { exact: true }).nth(3)
    8) <label class="peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-[10px] font-semibold text-emerald-700 uppercase tracking-tight w-8 shrink-0">OD</label> aka getByText('OD', { exact: true }).nth(4)
    9) <label class="peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-[10px] font-semibold text-teal-700 uppercase tracking-tight w-8 shrink-0">OD</label> aka getByText('OD', { exact: true }).nth(5)

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('OD')
