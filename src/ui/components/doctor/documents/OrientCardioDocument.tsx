...
interface OrientCardioDocumentProps {
	patient: { surname: string; name: string; dob: string };
}

// PDF Generation Function
export const generateOrientCardioPDF = async (
	...
	// Description
	const descriptionText = "Cher(e) confrère Je vous adresse le(la) sus-nommé(e) pour examen cardio vasculaire dans un but diagnostique étiologique";
...
// UI Component
const OrientCardioDocument: React.FC<OrientCardioDocumentProps> = ({
	patient,
}) => {
	return (
		<div className="p-4">
			<div className="mb-4 p-3 bg-blue-50/50 rounded-lg border border-blue-100">
				...
				<p className="text-xs italic text-muted-foreground">
					Cher(e) confrère Je vous adresse le(la) sus-nommé(e) pour examen cardio vasculaire dans un but diagnostique étiologique
				</p>
			</div>
		</div>
	);
...
export default memo(OrientCardioDocument);
