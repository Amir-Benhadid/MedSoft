...
interface CNASOCTGDocumentProps {
	patient: { surname: string; name: string; dob: string };
}

// PDF Generation Function
export const generateCNASOCTGPDF = async (
	...
	// Description
	const descriptionText = "Le(la) sus-nommé(e) présente à l'examen du jour une optic coherence tomography Indication: Glaucome chronique Cotation: K80";
...
// UI Component
const CNASOCTGDocument: React.FC<CNASOCTGDocumentProps> = ({
	patient,
}) => {
	return (
			...
			<Card className="bg-card rounded-xl p-3 border border-border shadow-sm">
				<CardContent className="p-0 space-y-2">
					...
					<p className="text-xs text-muted-foreground italic">
						Le(la) sus-nommé(e) présente à l'examen du jour une optic coherence tomography Indication: Glaucome chronique Cotation: K80
					</p>
				</CardContent>
			</Card>
		</div>
...
export default memo(CNASOCTGDocument);
