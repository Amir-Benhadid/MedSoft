...
interface CNASECHODocumentProps {
	patient: { surname: string; name: string; dob: string };
}

// PDF Generation Function
export const generateCNASECHOPDF = async (
	...
	// Description
	const descriptionText = "Le(la) sus-nommé(e) présente à l'examen du jour une échographie oculaire mode B Indication: HDV Cotation: K30";
...
// UI Component
const CNASECHODocument: React.FC<CNASECHODocumentProps> = ({
	patient,
}) => {
	return (
			...
			<Card className="bg-card rounded-xl p-3 border border-border shadow-sm">
				<CardContent className="p-0 space-y-2">
					...
					<p className="text-xs text-muted-foreground italic">
						Le(la) sus-nommé(e) présente à l'examen du jour une échographie oculaire mode B Indication: HDV Cotation: K30
					</p>
				</CardContent>
			</Card>
		</div>
...
export default memo(CNASECHODocument);
